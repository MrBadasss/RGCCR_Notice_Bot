...
def ensure_file_exists(file_path, default_content=""):
    """Ensures the given file exists, creating it if missing."""
...
def read_cached_notices():
    ...
    ensure_file_exists(CACHE_FILE)  # Ensure the cache file exists
...
def read_last_modified_time():
    """Reads the last modified time from the file."""
    ensure_file_exists(LAST_MODIFIED_FILE)  # Ensure last modified file exists
    with open(LAST_MODIFIED_FILE, "r", encoding="utf-8") as file:
        return file.read().strip()

def write_last_modified_time(last_modified):
    """Writes the last modified time to the file."""
    ensure_file_exists(LAST_MODIFIED_FILE)  # Ensure last modified file exists
    with open(LAST_MODIFIED_FILE, "w", encoding="utf-8") as file:
